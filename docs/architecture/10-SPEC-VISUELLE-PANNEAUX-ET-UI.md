...
> Tenue — **Barre flottante de sélection contextuelle** : centrée à `bottom: 12px`, fond `#1a1a1a`, bordure `#333333`, rayon 6 px, padding `4px 8px` ; compteur accordé (`1 sélectionné` / `11 sélectionnés`) avec séparateur `#2a2a2a` ; action **Verrouiller** (cadenas ouvert ; `Verrouillé`, cadenas fermé, sur fond `#2a1a1a`, filet `#553333`, texte `#f87171`) ; action **Supprimer** (corbeille). Les paths des deux icônes sont ceux de la référence. Mise en page pure tenue par `ui/action_bar/tests.rs`, rendu tenu par l'empreinte de la scène `witness_selected`.
>
> **Deux écarts.** Pas d'ombre portée : le rastériseur n'en a pas encore, et aucune autre surface flottante du Rust n'en porte — ce sera un travail commun aux panneaux, aux toasts et à celle-ci. Et pas de **tags** : la rangée de pastilles qui apparaît quand une seule image est sélectionnée demande un champ de saisie, donc un chantier à part.
