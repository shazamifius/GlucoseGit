...
use habit::draw_card_brume;
pub(super) use habit::{fond_du_canevas, forme_de_la_carte, teinte_de_carte, Contenants};
...
pub(super) struct TextCard<'a> {
    ...
    pub body: &'a str,
    pub tint: (u8, u8, u8),
    /// Ce que la carte peint sous son texte (LUEUR-3) : le fond du canevas vu au travers des
    /// membranes qui la contiennent ([`Contenants`]).
    pub fond: (u8, u8, u8),
    pub selected: bool,
    pub editing: Option<&'a TextEditSession>,
    /// Les passages qui y brillent (PASSAGE-2) : la mise en page leur ouvre la place, et la
    /// carte peint leur fond, leur liseré et leurs lettres à la teinte.
    pub eclaires: Option<super::passages::Eclaires<'a>>,
}
...
fn poser(ctx: &Pass, card: &TextCard) -> Option<(TextLayout, CardLayout, (f32, f32))> {
    // La mise en page ouverte pour ses passages qui brillent (PASSAGE-2) : la coupe des lignes
    // est la même, donc la hauteur aussi.
    let text = super::passages::mise_en_page(
        (ctx.typography, ctx.math),
        (card.body, card.size.0),
        card.mode(),
        card.eclaires.as_ref(),
    );
    let layout =
...
fn draw_card_body(
    ...
    // Le fond et le liseré des passages qui brillent, sous leurs lettres (PASSAGE-2).
    if let Some(e) = card.eclaires.filter(|_| card.editing.is_none()) {
        let passages = super::passages::troncons(
            (ctx.typography, ctx.math),
            (text, card.body, card.size.0),
            e.plages,
        );
        let pose = super::passages::Pose {
            origine: card.origin,
            vp: ctx.vp,
            echelle: ctx.scale,
        };
        super::passages::peindre_les_cadres(pixmap, pose, &passages, (e.teinte, e.style));
    }
...
fn draw_text_line(
    ...
    let text_ink = ink_of(line.kind, ctx.theme);
    let ink = Ink {
        text: text_ink,
        link: ctx.theme.link,
        marker: marker_ink(ctx.math, ctx.theme, line, card.body, card.editing.is_some()),
        eclat: card.eclaires.map_or(text_ink, |e| {
            let (r, g, b) = e.teinte;
