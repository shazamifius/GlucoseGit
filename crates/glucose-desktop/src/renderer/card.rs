...
pub(super) fn fond_du_canevas(theme: &Theme) -> (u8, u8, u8) {
...
pub(super) struct Contenants<'a> {
    fond: (u8, u8, u8),
    membranes: &'a [Membrane],
}

impl<'a> Contenants<'a> {
    pub(super) fn nouveaux(theme: &Theme, membranes: &'a [Membrane]) -> Self {
        Self {
    ...
    pub(super) fn fond_en(&self, (x, y): (f32, f32)) -> (u8, u8, u8) {
        let (r, g, b) = self.fond;
