...
fn test_resize_1_the_right_handle_of_an_image_keeps_its_left_edge_fixed() {
    ...
    assert!(
        ...
        "l'appui sur la poignée ouvre le geste"
    );
    ...
    assert!(
        approx(after.height, 100.0),
        "un côté ne change qu'une dimension : {after:?}"
    );
...
fn test_resize_1_every_handle_of_an_image_anchors_the_opposite_side() {
    for handle in Handle::ALL {
        ...
        assert!(
            approx(l, if handle.moves_left() { l0 + 20.0 } else { l0 }),
            "{name} gauche {l}"
        );
        assert!(
            approx(r, if handle.moves_right() { r0 + 20.0 } else { r0 }),
            "{name} droite {r}"
        );
        assert!(
            approx(t, if handle.moves_top() { t0 - 10.0 } else { t0 }),
            "{name} haut {t}"
        );
        assert!(
            approx(b, if handle.moves_bottom() { b0 - 10.0 } else { b0 }),
            "{name} bas {b}"
        );
    }
}
...
fn test_resize_3_an_image_corner_keeps_its_ratio_and_shift_frees_it() {
    ...
    assert!(
        approx(kept.width / kept.height, 2.0),
        "rapport conservé : {kept:?}"
    );
    assert!(kept.width > 200.0 && kept.height > 100.0, "{kept:?}");
    assert!(
        approx(kept.left, -100.0) && approx(kept.top, -50.0),
        "ancre haut-gauche : {kept:?}"
    );
    ...
    assert!(
        approx(freed.height, kept.height),
        "Shift libère le rapport : {freed:?}"
    );
}
...
fn test_resize_2_an_image_cannot_be_pulled_below_its_minimum_nor_flipped() {
    ...
    let after = image_box(&app, "I1");
    assert!(
        approx(after.width, glucose_core::resize::MIN_IMAGE_SIDE),
        "{after:?}"
    );
    assert!(
        approx(after.height, glucose_core::resize::MIN_IMAGE_SIDE),
        "{after:?}"
    );
    assert!(
        approx(after.left, -100.0) && approx(after.top, -50.0),
        "l'ancre tient : {after:?}"
    );
}
...
fn test_undo_1_a_gesture_of_many_moves_is_one_undo_entry() {
    ...
    assert_eq!(
        ...
        entries + 1,
        "quarante mouvements, une entrée"
    );
...
fn test_a_click_on_a_handle_without_moving_leaves_no_undo_entry() {
    ...
    assert_eq!(
        ...
        "rien n'a changé : pas d'entrée d'undo, pas de « modifié »"
    );
}
...
fn test_escape_during_the_gesture_restores_the_start_size_and_leaves_no_entry() {
    ...
    assert!(
        image_box(&app, "I1").width > 200.0,
        "le nœud suit pendant le geste"
    );
    ...
    assert_eq!(
        image_box(&app, "I1"),
        start,
        "Échap rend la taille de départ"
    );
    assert!(app.resize_session.is_none());
...
fn test_text_fit_1_narrowing_a_card_reflows_its_text_and_its_height_follows() {
    ...
    app.store
        .add_annotation(&board, text_card("T1", -100.0, -40.0, 400.0, text));
    ...
    assert!(
        ...
        "plus de lignes, carte plus haute : {after:?} vs {start:?}"
    );
    assert!(
        approx(after.top, start.top),
        "la hauteur suit le texte, le haut ne bouge pas"
    );
    let expected = text_card_fit_height(&app.renderer.typography, &app.renderer.math, text, 180.0);
    assert!(
        approx(after.height, expected),
        "hauteur du document = hauteur reflué"
    );
}
...
fn test_text_fit_1_the_height_is_free_above_the_text_and_blocked_below() {
    ...
    let board = app.store.project.active_board_id.clone();
    app.store
        .add_annotation(&board, text_card("T1", -100.0, -40.0, 240.0, "hello"));
    app.fit_text_card_height("T1");
    render(&mut app);
...
fn test_text_fit_1_committing_a_text_edit_writes_the_fitted_height() {
    ...
    let board = app.store.project.active_board_id.clone();
    app.store
        .add_annotation(&board, text_card("T1", 0.0, 0.0, 240.0, "une ligne"));
    app.fit_text_card_height("T1");
    ...
    assert_eq!(
        app.store.undo_depth(),
        entries + 1,
        "une saisie = une entrée d'undo"
    );
    assert!(app.store.undo());
    assert!(
        approx(ann_box(&app, "T1").height, one_line),
        "Ctrl+Z rend le texte ET la hauteur d'avant"
    );
}
...
fn test_a_sticky_and_a_membrane_resize_from_any_side() {
    ...
    assert_eq!(
        sticky,
        AlignRect::new(-300.0, -230.0, 160.0, 150.0),
        "le bas reste, le haut monte"
    );
    ...
    assert_eq!(
        membrane,
        AlignRect::new(60.0, 50.0, 360.0, 300.0),
        "le coin haut-droit reste"
    );
}
...
fn test_snap_1_the_pulled_edge_snaps_to_a_neighbour_and_shows_a_guide() {
    ...
    assert_eq!(
        app.active_guides.x,
        Some(vec![350.0]),
        "le guide est visible pendant le geste"
    );
    release(&mut app);
    assert_eq!(
        app.active_guides,
    ...
    assert!(
        approx(free.left + free.width, 346.0),
        "aimant coupé : {free:?}"
    );
}
...
fn test_persist_1_a_resized_image_survives_save_and_reopen() {
    ...
    assert!(
        approx(resized.width, 300.0) && approx(resized.height, 150.0),
        "{resized:?}"
    );
...
fn test_text_fit_1_an_older_document_gets_its_card_heights_fitted_on_open() {
    ...
    assert!(approx(
        fitted,
        text_card_fit_height(
            &reopened.renderer.typography,
            &reopened.renderer.math,
            "un\ndeux\ntrois\nquatre",
            240.0
        )
    ));
