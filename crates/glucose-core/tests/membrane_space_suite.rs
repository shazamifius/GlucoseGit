...
fn test_the_deduced_scale_is_capped_at_one_and_floored_at_eight_percent() {
    ...
    assert_eq!(
        content_scale(MembraneMode::Minimized, m, 200.0, 100.0),
        1.0,
        "plafond"
    );
    assert_eq!(
        content_scale(MembraneMode::Minimized, m, 800.0, 600.0),
        0.5,
        "min des deux axes"
    );
    assert_eq!(
        content_scale(MembraneMode::Minimized, m, 40_000.0, 100.0),
        0.08,
        "plancher"
    );
    assert_eq!(
        content_scale(MembraneMode::Classic, m, 800.0, 600.0),
        1.0,
        "le classique ne réduit jamais"
    );
}
