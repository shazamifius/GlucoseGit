...
fn test_plan_board_stretch_empty_stretched_membrane_does_not_move() {
    let items = [memb("M", 0.0, 0.0, 200.0, 200.0, MembraneMode::Stretched, None)];
    assert_eq!(plan_board_stretch(&items).len(), 0);
}
...
fn test_plan_board_stretch_content_of_other_membrane_does_not_block() {
    let items = [
        memb("M", 0.0, 0.0, 200.0, 200.0, MembraneMode::Stretched, None),
        box_item("I", 0.0, 0.0, 300.0, 100.0, Some("M")),
        memb("AUTRE", 900.0, 900.0, 100.0, 100.0, MembraneMode::Minimized, None),
...
fn test_plan_board_stretch_parent_does_not_block_child() {
    let items = [
        ...
        memb("M", 0.0, 0.0, 200.0, 200.0, MembraneMode::Stretched, Some("P")),
        box_item("I", 0.0, 0.0, 300.0, 100.0, Some("M")),
    ];
    let out = plan_board_stretch(&items);
...
fn test_plan_board_stretch_sibling_blocks() {
    ...
    assert_eq!(out1.iter().find(|o| o.membrane_id == "M").unwrap().blocker_ids.len(), 0);
    ...
    assert_eq!(
        out2.iter().find(|o| o.membrane_id == "M").unwrap().blocker_ids,
        vec!["FRERE".to_string()]
    );
}
...
fn test_plan_board_stretch_nested_grow_before_parent() {
    let items = [
        memb("PARENTE", 0.0, 0.0, 150.0, 150.0, MembraneMode::Stretched, None),
        memb("FILLE", 0.0, 0.0, 100.0, 100.0, MembraneMode::Stretched, Some("PARENTE")),
...
fn test_plan_board_stretch_cycle_does_not_infinite_loop() {
    let items = [
        memb("A", 0.0, 0.0, 100.0, 100.0, MembraneMode::Stretched, Some("B")),
        memb("B", 0.0, 0.0, 100.0, 100.0, MembraneMode::Stretched, Some("A")),
    ];
